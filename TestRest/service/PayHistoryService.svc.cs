...
namespace TestRest
{
    ...
    public class PayHistoryService : IPayHistoryService
    {
        ...
        public List<MPay> GetPayment(string isPayment)
        {
            List<MPay> paymentList = new List<MPay>();
            {
                sqlCon = con.openConnection();
                SqlCommand cmd = new SqlCommand("select treat.Id_Treatment, rec.Id_Recipe, pat.Patient_Name, treat.Diagnose, doc.DoctorName, treat.DateTreatment, pay.PaymentDrug, pay.PaymentDoctor, pay.TotalPayment, pay.isPay from Patient.Treatment treat join Recipe.Recipe rec on treat.Id_Recipe = rec.Id_Recipe join Patient.Patient pat on treat.Id_Patient = pat.Id_Patient join Doctor.Doctor doc on treat.Id_Doctor = doc.Id_Doctor join Patient.Payment pay on pay.Id_Treatment = treat.Id_Treatment where pay.isPay = '" + isPayment + "'", sqlCon);
